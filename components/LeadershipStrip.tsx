...
const govtLeaders = [
  { role: "CM", color: "border-[#1f6f43]", img: "/images/leaders/cm.jpg" },
      ...
      <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-8 items-center">
        ...
        <div className="flex justify-center md:justify-end gap-10">
          {govtLeaders.map((item, i) => (
            <div key={i} className="flex flex-col items-center">
              <div className={`w-20 h-20 rounded-full ${item.color} border-2 overflow-hidden bg-[#1f6f43]/10 flex items-center justify-center shadow-sm`}>
                {item.img ? <img src={item.img} alt={item.role} className="w-full h-full object-cover" /> : <span className="text-2xl">👤</span>}
              </div>
              <p className="text-xs font-medium mt-2 text-gray-700">{item.role}</p>
            </div>
          ))}
